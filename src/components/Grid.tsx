...
export default function Grid({
  ...
  const flashMap = useRef<Map<number, { ts: number; isBomb: boolean }>>(
    new Map(),
  );
    ...
    if (prev.length === grid.length) {
      for (let i = 0; i < grid.length; i++) {
        if (grid[i].owner !== prev[i]?.owner) {
          flashMap.current.set(i, {
            ts: Date.now(),
            isBomb: lastBombIndices.includes(i),
          });
        }
      }
    }
  ...
  useEffect(() => {
    if (myBombs === 0) setBombMode(false);
  }, [myBombs]);
  ...
  const onMouseMoveCanvas = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      if (isPanning.current) {
        ...
        panRef.current = clampPan(
          panStart.current.px + dx,
          panStart.current.py + dy,
          zoomRef.current,
          canvas.width,
          canvas.height,
        );
    ...
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      if (isPanning.current) {
        ...
        e.currentTarget.style.cursor =
          zoomRef.current > 1 ? "grab" : "crosshair";
      }
    },
    [],
  );
