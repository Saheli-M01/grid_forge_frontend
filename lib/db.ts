...
// Lazy singleton — created on first use so DATABASE_URL is already set
...
const pool = {
  ...
  query: (text: string, values?: unknown[]) => getPool().query(text, values as any),
};
...
export async function renameUserCells(
  ...
  await pool.query(
    `UPDATE cells SET owner_name = $1 WHERE owner_id = $2`,
    [newName, ownerId]
  );
}
